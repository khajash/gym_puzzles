...
class RobotPuzzle(gym.Env):
	...
	def __init__(self, act_type='box', obs_depth=3, frameskip=4):
		...
		high = np.array([
			np.inf, np.inf, self.theta_threshold,  # Agent (location, theta, contact boolean)
			np.inf, np.inf, self.theta_threshold, np.inf,# Block 1 (rel location, rel theta, distance)
			# np.inf, np.inf, self.theta_threshold, # Block 2
			# np.inf, np.inf, self.theta_threshold, # Block 3
			np.inf, # contact boolean
			]) 
	...
	def _step(self, action):
		...
		for block in self.blocks:
			...
			if self.obs_type != 'image':
				self.state = [self.agent.worldCenter[0]*SCALE-x, 
